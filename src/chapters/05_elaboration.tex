...
In the Codemodel-Rifle framework, analyses are basically Cypher queries. If a defect’s pattern can be expressed with a Cypher query, it can be detected by the framework. This section details the analyses I implemented for Codemodel-Rifle without altering the graph, utilising only pattern-matching techniques.
...
\subsection{Globally Invalid Imports}

\subsection{Division By Zero (only for specific cases)}

\subsection{Misuse of Negative Integers as Function Arguments (only for specific cases)}

\subsection{Unreachable Code because of Exceptions (only for specific cases)}
...
\subsection{Division By Zero (for universal cases)}

\subsection{Misuse of Negative Integers as Function Arguments (for universal cases)}

\subsection{Unreachable Code because of Exception Thrown (for universal cases)}
