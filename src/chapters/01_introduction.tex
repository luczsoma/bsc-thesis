...
My prospective colleague and former research fellow, Dániel Stein created a graph-based static analysis framework for JavaScript (ECMAScript) called \emph{Codemodel-Rifle}.~\cite{stein-daniel-msc} The currently open-sourced project's source code is available on GitHub.~\cite{codemodel-rifle-github} The framework stores the parsed source code repository's each parsed file as a distinct property graph, called an \emph{Abstract Semantic Graph (ASG)}, and gives us an interface to run analyses via graph queries.
...
The thesis is structured as follows. \emph{\Cref{chapter:background}} presents the conception of static analysis, shortly summarizes the JavaScript and its static analysis approaches to be detailed in \Cref{chapter:relatedwork}, and gives insights to the previously mentioned background technologies of the \emph{Codemodel-Rifle} framework. It also provides an example which will accompany the reader throughout the thesis. \emph{\Cref{chapter:relatedwork}} specifies the currently known approaches and related work. \emph{\Cref{chapter:overview}} gives an overview of my approach of JavaScript static analysis using the \emph{Codemodel-Rifle} framework. \emph{\Cref{chapter:elaboration}} details the implementation of the analyses and the additional proceedings about processing multiple JavaScript modules related to each other. \emph{\Cref{chapter:evaluation}} demonstrates and evaluates the implemented analyses. \emph{\Cref{chapter:tresorit}} provides a case study with a closed-source, security-oriented industrial project from Tresorit. \emph{\Cref{chapter:conclusion}} concludes the thesis and presents the possible future work.
